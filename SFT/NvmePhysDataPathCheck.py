...
class NvmePhysDataPathCheck(TestBase):
    ...
    def Start(self):
	self.log.Print(NvmePhysDataPathCheck.section_str)
	self.Prepare()
	#if True:
	try:
            self.DiscoverDisks()
	...
	home_dir = self.config.Get('HOME_DIR')
	sas_hd_size = self.config.Get('NvmePhy_size')
...
if __name__ == '__main__':
    ...
    test = NvmePhysDataPathCheck(config, eventManager, log, comm)
    startTime=time.time() 
    result = test.Start()
    endTime=time.time()
    print "********************************************"
