...
class TestEngine:
	...
	if self.cycleIndex=="0":
        	...
        	self.log_filename = self.ongingLogfile+'.log'
	...
	FindHaltFlag=False
	    ...
	    if (not self.cycleIndex=="0") and (not FindHaltFlag):
		if line.find("HaltUUT")>=0 and line.find(self.cycleIndex)>=0:
			FindHaltFlag=True
			continue
		else:
			continue
	...
	#f = open(self.config.Get('HOME_DIR') + '/TestConfig/' + self.sequence ,'r')
        #lines = f.readlines()
	#results=os.system("ls ~/CMCC/OngoingTest/ | grep %s" %self.config.Get("PcbaSN"))
	results=os.popen("ls ~/CMCC/OngoingTest/ | grep %s" %self.config.Get("PcbaSN")).readlines()
	if len(results)>0:
		self.ongingLogfile=results[-1].strip()
		f = open(self.config.Get('HOME_DIR') + '/OngoingTest/' + results[-1].strip(),'r')
        	lines = f.readlines()
		if len(lines)>0:
			self.cycleIndex=lines[0].strip()
		
	#print "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXx"
	#print results
	#print "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXx"
	#if(os.path.isfile("TestEngine_3Cycle.py")) :
	...
	#remove ongoing file
        os.system('rm -rf '+self.config.Get('HOME_DIR')+"/OngoingTest/")
	print '++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++'
	print self.testResult
	print '++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++'

	header_complete_str = 'SilkRoad SFT: '  + self.testResult 
...
if __name__ == '__main__':
    ...
    result = testMain.Run()
    #result = testMain.GetCurrentStatus()
    #result="FAIL"
    if result[0:4] == 'FAIL':
