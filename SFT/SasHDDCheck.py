...
class SasHDDCheck(TestBase):
    ...
    def Start(self):
	self.log.Print(SasHDDCheck.section_str)
	#self.Prepare()
	#if True:
	try:
                self.TestHDD()
	...
	checkList=[]
	checklist_str=self.config.Get('HDD_CHECK_LIST')
	checklist_str.split(";")
	...
	for check in checkList:
		self.cmd_ssd_info="%s/tools/storage-tool-64  %s  -devicecount %s -action info" %(home_dir,check[0],check[1])
...
if __name__ == '__main__':
    ...
    test = SasHDDCheck(config, eventManager, log, comm)
    startTime=time.time() 
    result = test.Start()
    endTime=time.time()
    print "********************************************"
