//Compile with "g++ netloop-testtool3.cpp -luuid -lssl -lpthread"
//Dependencies: -
//  ping
//  iperf3
//  ifconfig
...
bool RunIperf(string physicalip1, string physicalip2, string virtualip1, string virtualip2, string runtime, double speed);
...
int main(int argc, char *argv[])
{
  ...
  //Show header
  WriteConsoleLine("network test tool 3");
  WriteConsoleLine("Version 1.5 Copyright Flextronics Penang");
  WriteConsoleLine("");
  ...
  if(param_iperfspeed.size() > 0)
  {
    ...
    {
      if(RunIperf(param_physicalip1, param_physicalip2, param_virtualip1, param_virtualip2, param_iperftime, min_iperfspeed) == true)
      {
        iperfresult = true;
        break;
      }
...
void *iperfServerThread(void *arg)
{
  ...
  string command = "iperf3 -s -f m -i 0";
  WriteConsoleLine("Action: Run \"" + command + "\"");
...
bool RunLEDs(string physicalip1, string physicalip2, string virtualip1, string virtualip2)
{
  ...
  string command = "iperf3 -B " + physicalip1 + " -c " + virtualip2 + " -t 1 -f m";
  //WriteConsoleLine("Action: Run \"" + command + "\"");
  vector<string> outlines = vector<string>();
  int exitcode;
  while( ! kbhit())
...
bool RunIperf(string physicalip1, string physicalip2, string virtualip1, string virtualip2, string runtime, double speed)
{
  ...
  string command = "iperf3 -B " + physicalip1 + " -c " + virtualip2 + " -t " + runtime + " -O 1 -i 1 -f m -T F -V";
  WriteConsoleLine("Action: Run \"" + command + "\"");
  vector<string> outlines = vector<string>();
  int exitcode;
  bool bret = RunProcessGetExitCodePrint(command, outlines, exitcode);
  ...
  for(int i = 0; i < outlines.size(); i++)
  {
    if(outlines[i].find("Test Complete. Summary Results:") != string::npos)
    {
        resultlineindex = i;
        break;
    }
  ...
  string senderrealspeedstring;
  for(int i = resultlineindex; i < outlines.size(); i++)
  {
    if((outlines[i].find("Mbits/sec") != string::npos) && (outlines[i].find("sender") != string::npos))
      senderrealspeedstring = outlines[i];
  }

  int index = senderrealspeedstring.find("Mbits/sec");
  ...
  index -= 1;
  while(senderrealspeedstring[index] == 32)
    index -= 1;
  while(senderrealspeedstring[index] != 32)
    index -= 1;
  index += 1;
  senderrealspeedstring = senderrealspeedstring.substr(index);
  double senderrealspeed = atof(senderrealspeedstring.c_str());
  if(senderrealspeed >= speed)
  {
    WriteConsoleLine("Info: iperf forward test sender speed test passed");
  }
  else
  {
    WriteConsoleLine("Error: iperf forward test sender speed test failed");
    return false;
  }
  
  string receiverrealspeedstring;
  for(int i = resultlineindex; i < outlines.size(); i++)
  {
    if((outlines[i].find("Mbits/sec") != string::npos) && (outlines[i].find("receiver") != string::npos))
      receiverrealspeedstring = outlines[i];
  }

  index = receiverrealspeedstring.find("Mbits/sec");
  if(index == string::npos)
  {
    WriteConsoleLine("Error: Failed to get speed");
    return false;
  ...
  index -= 1;
  while(receiverrealspeedstring[index] == 32)
    index -= 1;
  while(receiverrealspeedstring[index] != 32)
    index -= 1;
  index += 1;
  receiverrealspeedstring = receiverrealspeedstring.substr(index);
  double receiverrealspeed = atof(receiverrealspeedstring.c_str());
  if(receiverrealspeed >= speed)
  {
    WriteConsoleLine("Info: iperf forward test receiver speed test passed");
  }
  else
  {
    WriteConsoleLine("Error: iperf forward test receiver speed test failed");
    return false;
  }
  ...
  command = "iperf3 -B " + physicalip1 + " -c " + virtualip2 + " -t " + runtime + " -O 1 -i 1 -f m -R -T R -V";
  WriteConsoleLine("Action: Run \"" + command + "\"");
  outlines = vector<string>();
  bret = RunProcessGetExitCodePrint(command, outlines, exitcode);
  ...
  index = senderrealspeedstring.find("Mbits/sec");
  if(index == string::npos)
  {
    WriteConsoleLine("Error: Failed to get speed");
    return false;
  ...
  index += 1;
  senderrealspeedstring = senderrealspeedstring.substr(index);
  senderrealspeed = atof(senderrealspeedstring.c_str());
  if(senderrealspeed >= speed)
  {
    WriteConsoleLine("Info: iperf reverse test sender speed test passed");
  }
  else
  {
    WriteConsoleLine("Error: iperf reverse test sender speed test failed");
    return false;
  }
  
  receiverrealspeedstring = "";
  for(int i = resultlineindex; i < outlines.size(); i++)
  {
    if((outlines[i].find("Mbits/sec") != string::npos) && (outlines[i].find("receiver") != string::npos))
      receiverrealspeedstring = outlines[i];
  ...
  index += 1;
  receiverrealspeedstring = receiverrealspeedstring.substr(index);
  receiverrealspeed = atof(receiverrealspeedstring.c_str());
  if(receiverrealspeed >= speed)
  {
    WriteConsoleLine("Info: iperf reverse test receiver speed test passed");
  }
  else
  {
    WriteConsoleLine("Error: iperf reverse test receiver speed test failed");
    return false;
  }
